...
impl Position {
    fn notional_value(&self) -> Decimal {
        Decimal::from(self.quantity) * self.cost_basis
    }
}
...
impl Portfolio {
    ...
    fn rebalance(&mut self, target_weights: &DataFrame, prices: &DataFrame) -> PyResult<()> {
            ...
            .iter()
        {
            ...
            let price = *price_map.get(ticker).ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Price not found")
            })?;
...
fn initialize_backtester(model_state: PyDataFrame, model_insights: PyDataFrame) -> PyResult<bool> {
    ...
    let insights_range = insights_df
        .lazy()
        .select([
...
fn simulate_portfolio(
    ...
    for date in dates.into_iter().flatten() {
        ...
        let day_state = state_df
            .clone()
            .lazy()
            .filter(col("date").eq(lit(date.date_naive())))
            .collect()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
        let day_insights = model_insights
            .0
            .clone()
            .lazy()
